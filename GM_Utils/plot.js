...
JB.plot = function(feld,xstr,ystr) {
	...
	this.frame = function(x0,y0,xl,yl) {
		ifeld.innerHTML = "";
		var cp = JB_makediv(ifeld,"",0,0,10,10);
		cp.innerHTML = "<a href='http://www.j-berkemeier.de' title='Plot 4. 9. 2015'>X</a>";
		cp.style.zIndex = "100";
		cp.style.opacity = "0";
		...
		if(this.xscale60 && tx<.75) {
			...
			for(var x=mxmin;x<=xmax;x+=tx) {
				var xx = (x-xmin)*xfak;
				gr.linie(xx,0,xx,gr.h,this.gridcol);
				//Melanie: changed from JB.toString(x) to (x-xmin).toFixed(1)
				var label = (xmin<0)?x:(x-xmin);
				if(xl.length && xx<(gr.w-5) && xx>5) xlabel.text(xx,xlabel.h-2,".8em",this.labelcol,(label).toFixed(1),"mo");
			}
		}
	...
	this.plot = function(a,col) {
		...
		if (col==""){
			...
			do{
				var arr=[];
				var speed = a[i].v;
				var color = JB.fixedColor(JB.getTimeInSec(a[i]));
                //cols[j] = (color==="")?JB.getColor(speed):color;
                col= (color==="")?JB.getColor(speed):color;
				// col= (color==="")?JB.getColor(speed):color;//(a[i].v<2.5)?"rgb(0,0,255)":"rgb(0,255,0)"; // noch anpassen
				var amountTrackPointsInOtherState = 0;	
				while(i<l && amountTrackPointsInOtherState<2){
                    var newColor = JB.fixedColor(JB.getTimeInSec(a[i]));
                    var newCol = (newColor==="")?JB.getColor(a[i].v):newColor;
					if (newCol != col ) amountTrackPointsInOtherState++;
					else amountTrackPointsInOtherState = 0;
				...
				if(this.fillopac>0) {
					...
					arr.push({x:arr[arr.length-1].x,y:y0});
					arr.push({x:arr[0].x,y:y0});
	...
	this.markeron = function(a,callback_over,callback_out,callback_move,callback_click,callback_dblclick,markertype) {
		var dieses = this;
		var posx=0,offx;
		this.mele.onmouseover = this.mele.ontouchstart = function(e) {
			...
			dieses.mele.ondblclick = function(e) {
				if(!e) e = window.event;
				e.cancelBubble = true;
				if(e.stopPropagation) e.stopPropagation();
                if(e.targetTouches && e.targetTouches[0] && e.targetTouches[0].clientX) posx = e.targetTouches[0].clientX;
				...
				pi = dieses.getPolylinePos(posx,a);
				dieses.setmarker(a[pi],markertype);
				if(callback_dblclick && typeof(callback_dblclick)=="function") callback_dblclick(a[pi]);
